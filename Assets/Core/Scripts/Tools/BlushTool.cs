...
{
public class BlushTool : BaseToolBehaviour, IColorableTool
{
    [Header("References")]
    [SerializeField]
    ...
    protected override async UniTask PickUp()
    {
        ...
        await handRect.DOPunchPosition(new Vector3(20f, 20f, 0f), 0.6f, 5, 0.5f).ToUniTask();
    ...
    protected override async UniTask WaitForPlayerDrag()
    {
        ...
        {
            if (!controller.DragController.FaceZone.ContainsCheeks(screenPos))
                return;
            MoveToFaceAndApply(tcs).Forget();
        }
    ...
    {
        controller.DragController.DisableDrag();
        Vector3 targetPos = controller.DragController.FaceZone.GetCheeksRectPosition() +
    ...
    private async UniTask Apply()
    {
        ...
        AudioEvents.BlushApplied();
        await handRect.DOPunchPosition(new Vector3(0, 25f, 0), 1f, 5, 0.5f).ToUniTask();
        controller.FaceVisuals.ApplyBlush(_selectedSprite);
    }
